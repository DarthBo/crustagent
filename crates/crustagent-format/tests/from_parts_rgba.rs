...
fn build() -> AcsFile {
    ...
    let images = vec![solid(2, 2, [255, 0, 0, 255]), solid(2, 2, [0, 255, 0, 255])];
    let anim = Animation {
        name: "Wave".into(),
...
fn transparent_source_leaves_canvas_clear() {
    ...
    let f = file
        .composite_frame(&file.animation("Idle").unwrap().frames[0], None)
        .unwrap();
    assert!(
        f.is_fully_transparent(),
        "transparent source keeps the frame clear"
    );
}
...
fn semi_transparent_over_blends() {
    ...
    let f = file
        .composite_frame(&file.animation("Blend").unwrap().frames[0], None)
        .unwrap();
    ...
    assert!(
        (120..=136).contains(&f.pixels[1]),
        "green blends toward ~128, got {}",
        f.pixels[1]
    );
}
